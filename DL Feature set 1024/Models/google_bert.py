...
class HateSpeechBERT(nn.Module):
    ...
    def __init__(self, num_embeddings:int=960, num_encodings:int=64, input_encodings:int=1024, num_labels:int=1):
        ...
        self.transformer = AutoModelForMaskedLM.from_pretrained("google-bert/bert-large-uncased")
        self.classifier = nn.Linear(in_features=1024, out_features=num_embeddings, bias=True)
